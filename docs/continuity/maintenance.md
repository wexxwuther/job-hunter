# Maintenance, job-hunter

Recurring tasks + freshness checks. Each has a cadence, a trigger, and the exact commands or
manual steps. Run on the schedule even if nothing seems broken, staleness compounds quietly.
...
**Cadence:** every 90 days from `_meta.json.last_review_due` (current v5.2.0 value: 2026-08-19 → next 2026-11-17 → …)
...
## Push repo changes to the installed skill
...
# 1. Confirm clean + audit/tests pass before deploying
git status --short
python C:\Users\Owner\.claude\skills\self-improving-skills\scripts\audit_existing.py --skill .
python -m pytest tests\ -q            # expect 201 passed (v5.2.0)

# 2. Run the public-repo installer, deploys whatever is on main to ~/.claude/skills/job-hunter/
powershell -ExecutionPolicy Bypass -File install\install.ps1
#   (macOS/Linux: bash install/install.sh)

# 3. Verify installed copy
python C:\Users\Owner\.claude\skills\self-improving-skills\scripts\ci_check.py C:\Users\Owner\.claude\skills\job-hunter
```

`install/install.ps1` (Windows) and `install/install.sh` (macOS/Linux) are the canonical install
paths for v5+. They deploy the no-version-suffix `job-hunter` install. Per-harness install guides
live at `install/claude-code.md`, `install/codex.md`, `install/openclaw.md`, `install/hermes.md`.
